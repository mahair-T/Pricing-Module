...
# ============================================
# Use Loaded City Map (Source of Truth) for Dropdowns
...
# Ensure defaults exist in list or fallback to first item
default_pickup = 'Jeddah' if 'Jeddah' in pickup_cities_en else (pickup_cities_en[0] if pickup_cities_en else None)
default_dest = 'Riyadh' if 'Riyadh' in dest_cities_en else (dest_cities_en[0] if dest_cities_en else None)
...
with tab1:
    ...
    with col1:
        ...
        pickup_en = st.selectbox("Pickup City", options=pickup_cities_en,
            index=pickup_cities_en.index(default_pickup) if default_pickup else 0, key='single_pickup')
        pickup_city = to_arabic_city(pickup_en)

    with col2:
        dest_en = st.selectbox("Destination City", options=dest_cities_en,
            index=dest_cities_en.index(default_dest) if default_dest else 0, key='single_dest')
