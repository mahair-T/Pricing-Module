...
def load_city_normalization():
    ...
    variant_to_canonical_aggressive = {}
    variant_to_region = {}
    canonical_to_region = {}
    variant_to_province = {}
    canonical_to_province = {}
    ...
    if os.path.exists(norm_path):
        try:
            ...
            for _, row in df.iterrows():
                ...
                v_agg = normalize_aggressive(variant)
                if v_agg: variant_to_canonical_aggressive[v_agg] = canonical
                ...
                if province:
                    ...
                    if canonical not in canonical_to_province: canonical_to_province[canonical] = province
...
def normalize_city(city_raw):
    ...
    # --- CHECK RUNTIME VARIANTS (From user approval) ---
...
# --- NEW: FUZZY MATCH HELPER ---
def find_best_match(city, choices, cutoff=0.8):
    """Find best fuzzy match with confidence > 80%."""
    matches = difflib.get_close_matches(city, choices, n=1, cutoff=cutoff)
    if matches:
        score = difflib.SequenceMatcher(None, city, matches[0]).ratio()
        return matches[0], round(score, 2)
    return None, 0.0
# -------------------------------
...
CITIES_WITHOUT_REGIONS = [] # Skipped validation loop for brevity, kept structure
...
def calculate_prices(pickup_ar, dest_ar, requested_vehicle_ar, distance_km, lane_data=None, 
    ...
    return {
        'buy_price': buy_rounded, 'sell_price': sell_rounded, 'base_flatbed_price': base_price,
        'ref_sell_price': round_to_nearest(ref_sell, 50), 'ref_sell_source': ref_src, 
        'rental_cost': calculate_rental_cost(distance_km), 'target_margin': f"{margin:.0%}", 
        'backhaul_probability': bh_prob, 'backhaul_ratio': round(bh_ratio, 2) if bh_ratio else None,
...
def price_single_route(pickup_ar, dest_ar, vehicle_ar=None, commodity=None, weight=None):
    if not vehicle_ar: vehicle_ar = DEFAULT_VEHICLE_AR
    lane = f"{pickup_ar} → {dest_ar}"
    ...
    res = {
        ...
        'Is_Rare_Lane': pricing['recent_count'] == 0
    }
    # Add stats placeholders for UI compatibility
    res.update({'Hist_Count': 0, 'Hist_Min': 0, 'Hist_Median': 0, 'Hist_Max': 0, 
                f'Recent_{RECENCY_WINDOW}d_Count': pricing['recent_count'], 
                f'Recent_{RECENCY_WINDOW}d_Min': 0, f'Recent_{RECENCY_WINDOW}d_Median': 0, f'Recent_{RECENCY_WINDOW}d_Max': 0,
                'Hist_Sell_Min': 0, 'Hist_Sell_Median': 0, 'Hist_Sell_Max': 0,
                f'Recent_{RECENCY_WINDOW}d_Sell_Min': 0, f'Recent_{RECENCY_WINDOW}d_Sell_Median': 0, f'Recent_{RECENCY_WINDOW}d_Sell_Max': 0})
    return res
...
def lookup_route_stats(pickup_ar, dest_ar, vehicle_ar=None, dist_override=None, check_history=False,
    ...
    if dist_override and dist_override > 0: dist, dist_src = dist_override, 'User Provided'
    else: dist, dist_src = get_distance(pickup_ar, dest_ar, lane_data, immediate_log=False, check_history=check_history)
    
    pricing = calculate_prices(pickup_ar, dest_ar, vehicle_ar, dist, lane_data, pickup_region_override, dest_region_override, weight)
    
    return {
        'From': to_english_city(pickup_ar), 'To': to_english_city(dest_ar),
        'Vehicle': to_english_vehicle(vehicle_ar), 'Weight_Tons': weight if weight else 25.0,
        'Distance': int(dist) if dist else 0, 'Distance_Source': dist_src,
...
st.caption(f"ML-powered pricing | Domestic | {' | '.join(model_status)}")
...
with tab1:
    ...
    if st.button("🎯 Generate Pricing", type="primary"):
        res = price_single_route(to_arabic_city(pickup_en), to_arabic_city(dest_en), to_arabic_vehicle(vehicle_en))
        st.info(f"{pickup_en} → {dest_en} | {res['Vehicle_Type']} | {res['Distance_km']} km")
        c1, c2, c3, c4 = st.columns(4)
        c1.metric("🛒 BUY PRICE", f"{res['Buy_Price']:,} SAR" if res['Buy_Price'] else "N/A")
        c2.metric("💵 SELL PRICE", f"{res['Rec_Sell_Price']:,} SAR" if res.get('Rec_Sell_Price') else "N/A")
        c3.metric("Margin", res['Target_Margin'])
        c4.metric("Model", res['Model_Used'])
...
with tab2:
    ...
    if upl:
        ...
        for i, v in enumerate(vehicle_types_en):
            with v_cols[i%4]:
                if st.checkbox(v, value=(v=='Flatbed Trailer')): selected_vehicles.append(v)
        ...
        if st.session_state.bulk_stage == 'scanning':
            ...
            for i in range(len(r_df)):
                row = r_df.iloc[i]
                p_raw, d_raw = str(row.iloc[0]).strip(), str(row.iloc[1]).strip()
                _, p_ok = normalize_city(p_raw)
                _, d_ok = normalize_city(d_raw)
                if not p_ok: unmatched.add(p_raw)
                if not d_ok: unmatched.add(d_raw)
        ...
        if st.session_state.bulk_stage == 'fuzzy_review':
            st.warning("⚠️ Unmatched Cities Found")
            st.caption("Please review suggested matches. Check 'Approve' to map them permanently. Leave unchecked to treat as new cities.")
            
            # Generate fuzzy matches only once
            ...
            if st.button("✅ Confirm Matches & Proceed"):
                # Process approvals
                ...
                if new_variants:
                    ...
                    st.success(f"Logged {len(new_variants)} new variants!")
                
                # Determine remaining unmatched
                remaining = []
                for city in st.session_state.unmatched_list:
                    # check if it was just approved
                    if city not in st.session_state.runtime_variants:
        ...
        # STAGE 3: COORDINATE INPUT (For new cities)
        if st.session_state.bulk_stage == 'coord_input':
            st.info("🌍 New Cities Detected: Please provide Coordinates")
            st.caption("These cities have no match. Enter Lat/Long so we can identify their province for pricing.")
            
            if 'coord_df' not in st.session_state:
                st.session_state.coord_df = pd.DataFrame([
                    {'City': c, 'Lat': 0.0, 'Lon': 0.0} for c in st.session_state.remaining_unmatched
                ])
                
            coords_edited = st.data_editor(st.session_state.coord_df, use_container_width=True, hide_index=True)
            
            if st.button("Submit Coordinates & Calculate"):
                # Store coords in a temp dict for this run
        ...
        if st.session_state.bulk_stage == 'processing':
            ...
            for i in range(len(r_df)):
                ...
                # If still None (unmatched and no coord provided), keep raw
                if not p_ar: p_ar = p_raw
                if not d_ar: d_ar = d_raw
                
                # Check for temp coords
                p_reg_override, d_reg_override = None, None
                if 'temp_coords' in st.session_state:
                    if p_raw in st.session_state.temp_coords:
                        lat, lon = st.session_state.temp_coords[p_raw]
                        _, p_reg_override = get_province_from_coordinates(lat, lon)
                    if d_raw in st.session_state.temp_coords:
                        lat, lon = st.session_state.temp_coords[d_raw]
                        _, d_reg_override = get_province_from_coordinates(lat, lon)

                # Loop Vehicles
                vehicles = [to_arabic_vehicle(v) for v in selected_vehicles] if selected_vehicles else [DEFAULT_VEHICLE_AR]
                
                for v_ar in vehicles:
                    route_res = lookup_route_stats(p_ar, d_ar, v_ar, 
                                                   pickup_region_override=p_reg_override, 
                                                   dest_region_override=d_reg_override)
                    ...
                    if route_res['Distance'] == 0:
                        missing_dists.append({'From': route_res['From'], 'To': route_res['To'], 'Status': 'Missing'})
                
                prog.progress((i+1)/len(r_df))
            
            st.session_state.final_results = pd.DataFrame(res)
            st.session_state.missing_dists_df = pd.DataFrame(missing_dists).drop_duplicates()
            flush_matched_distances_to_sheet()
            st.session_state.bulk_stage = 'done'
            st.rerun()

        # STAGE 5: RESULTS & MISSING DISTANCES
        if st.session_state.bulk_stage == 'done':
            if 'missing_dists_df' in st.session_state and not st.session_state.missing_dists_df.empty:
                st.error(f"📏 {len(st.session_state.missing_dists_df)} Distances Missing")
                st.dataframe(st.session_state.missing_dists_df, use_container_width=True, hide_index=True)
                st.info("We have logged these to the MatchedDistances sheet. Please update them and click 'Refresh' in Admin.")
            
            st.subheader("📊 Final Results")
            st.dataframe(st.session_state.final_results, use_container_width=True, hide_index=True)
            
            st.download_button("📥 Download Results", st.session_state.final_results.to_csv(index=False), "results.csv", "text/csv")
    ...
    if 'final_results' in st.session_state and not st.session_state.final_results.empty:
        res_df = st.session_state.final_results
        st.markdown("---")
        st.subheader("Save 🚀")
        ...
        if rfq_sheet_url:
            ...
            sheet_name = st.text_input("Sheet Name", value=default_sheet_name, key="rfq_sheet_name")
            
            col1, col2 = st.columns(2)
            with col1:
                upload_clicked = st.button("☁️ Upload to Google Sheet", type="secondary", key="upload_to_rfq", use_container_width=True)
            with col2:
                st.download_button("📥 Download Results CSV", res_df.to_csv(index=False), "pricing_results.csv", "text/csv", type="secondary", key="download_results_csv", use_container_width=True)

            if upload_clicked:
                if not sheet_name or sheet_name.strip() == '': st.error("Please enter a sheet name")
                else:
                    ok, msg = upload_to_rfq_sheet(res_df, sheet_name.strip())
                    if ok: 
                        st.success(msg)
...
with st.expander("📏 Admin: Update Distances from Google Sheets"):
    ...
    with c1: refresh_clicked = st.button("🔍 Refresh Sheet Data", use_container_width=True)
    with c2: import_clicked = st.button("🔄 Import to Pickle", type="primary", disabled=len(resolved)==0, use_container_width=True)
    with c3:
        pkl_path = os.path.join(APP_DIR, 'model_export', 'distance_matrix.pkl')
        if os.path.exists(pkl_path):
